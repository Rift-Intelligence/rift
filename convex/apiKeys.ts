 ...
 * use it as `Authorization: Bearer <key>` against /api/chat to drive the full
 * RIFT agent (every tool, every purpose) from their own terminal/scripts —
 * the exact same code path the web app uses, just authenticated differently.
 *
...
/**
 * Create a new key. Premium-gated: requires an active pro/ultra subscription
...
export const resolveForBackend = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Re-verify premium status live — a key survives a lapsed subscription in
    ...
    return { userId: row.user_id, tier: premiumTier };
  },
});
