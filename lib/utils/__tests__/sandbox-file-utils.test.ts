...
describe("desktop-local sandbox file helpers", () => {
    ...
    expect(sandboxFiles).toEqual([
      {
        ...
        localPath: "/tmp/rift-upload/report.pdf",
      },
    ]);
    ...
    expect(
      messages[0].parts?.some(
        (part: any) =>
          part.type === "text" &&
          part.text ===
            '<attachment filename="report.pdf" local_path="/tmp/rift-upload/report.pdf" />',
      ),
    ).toBe(true);
  });
  ...
  it("retries url uploads in a writable directory when /tmp is not writable", async () => {
    ...
    const downloadFromUrl = jest
      ...
      .mockRejectedValueOnce(
        new Error(
          "Failed to download file: mkdir: cannot create directory '/tmp/rift-upload': Permission denied",
        ),
      )
      .mockResolvedValueOnce(undefined);
    const run = jest.fn().mockResolvedValue({
      exitCode: 0,
      stdout: "/home/alice/rift-upload/report.pdf",
      stderr: "",
    });
    ...
    try {
      ...
      expect(result).toEqual({
        ...
        pathRewrites: [
          {
            from: "/tmp/rift-upload/report.pdf",
            to: "/home/alice/rift-upload/report.pdf",
          },
        ],
      });
      ...
      expect(downloadFromUrl).toHaveBeenCalledWith(
        "https://example.com/report.pdf",
        "/home/alice/rift-upload/report.pdf",
      );
    } finally {
  ...
  it("rewrites attachment tags after upload path fallback", () => {
    const messages = [
      {
        ...
        parts: [
          {
            type: "text",
            text: '<attachment filename="report.pdf" local_path="/tmp/rift-upload/report.pdf" />',
          },
        ],
      },
    ] as UIMessage[];
    ...
    expect(rewritten[0].parts?.[0]).toMatchObject({
      text: '<attachment filename="report.pdf" local_path="/home/alice/rift-upload/report.pdf" />',
    });
  });
});
